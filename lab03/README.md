...
2. **Storing `isLeap` and `isThirtyDayMonth` boolean fields**. Calculating whether the year is a leap year or the month a 30 day month several times over an object's lifetime is inefficient; these will now be calculated once during instantiation. `isLeapYear()` and `isThirtyDayMonth()` were removed and methods were changed to use the stored precomputed values instead.<br>**N.B.:** `isEndOfMonth` could be replaced for a similar reason, but it does help code readability and is useless to calculate and store for Date objects not expected to call `nextDate`. 
